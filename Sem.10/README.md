...
- предварително определен ограничен размер (най-често в диапазона 1-8MB)
- състои се от фиксирани блокове памет (stack frames), чиито големини се смятат спрямо големините на променливите в тях и следователно трябва да са известни по време на компилация.
...
- по-бавна от Стековата памет конкретно за заделяне на обекти.

- НЯМА фиксиран размер и расте динамично по време на изпълнение според нуждите на програмата и ограниченията на системата.
- Ще съдържа нашите масиви и обекти без предварително да е известна големината.
...
```
При приключване на програмата всяка не ръчно освободена памет ще бъде автоматично освободена от повечето модерни операционни системи.
Това не означава, че не трябва да я освобождаваме ние! ОС се грижи за Стека, а ние имаме отговорност за Heap-a!
...
Примери:
 ...
 - Да се напише функция, която приема стринг и връща два стринга с ТОЧНА големина. Първият да бъде съставен само от малките букви, а другият да бъде съставен само от главните букви на подадения.
...
**Задача 7:** Напишете функция, която приема като параметър два едномерни сортирани целочислени масива с елементи от тип int и връща като резултат трети масив
